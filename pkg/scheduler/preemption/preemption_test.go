...
func TestWorkloadFits(t *testing.T) {
	...
	flavors := []*kueue.ResourceFlavor{
		utiltesting.MakeResourceFlavor("default").Obj(),
		utiltesting.MakeResourceFlavor("alternate").Obj(),
	}
	...
	tests := map[string]struct {
		...
		"workload fits within quota": {
			...
			admitted: []kueue.Workload{
				*utiltesting.MakeWorkload("existing-wl", "").
					Queue("test-queue").
					Request(cpu, "3").
					Request(memory, "3Gi").
					ReserveQuota(utiltesting.MakeAdmission("test-cq").Assignment(cpu, "default", "3").Assignment(memory, "default", "3Gi").Obj()).
					Obj(),
			},
			targetCQ: "test-cq",
			incomingResources: map[corev1.ResourceName]string{
		...
		"workload exceeds available quota": {
			...
			admitted: []kueue.Workload{
				*utiltesting.MakeWorkload("existing-wl", "").
					...
					Request(cpu, "8").
					Request(memory, "8Gi").
					ReserveQuota(utiltesting.MakeAdmission("test-cq").Assignment(cpu, "default", "8").Assignment(memory, "default", "8Gi").Obj()).
					Obj(),
			},
			targetCQ: "test-cq",
			incomingResources: map[corev1.ResourceName]string{
		...
		"workload needs borrowing and borrowing is allowed": {
			clusterQueues: []*kueue.ClusterQueue{
				...
				utiltesting.MakeClusterQueue("other-cq").
					Cohort("test-cohort").
					ResourceGroup(*utiltesting.MakeFlavorQuotas("default").
						Resource(cpu, "10", "10").
						Resource(memory, "10Gi", "10Gi").
		...
		"resource needs preemption, requires borrowing, borrowing not allowed": {
			...
			admitted: []kueue.Workload{
				*utiltesting.MakeWorkload("existing-wl", "").
					...
					Request(cpu, "8").
					Request(memory, "3Gi").
					ReserveQuota(utiltesting.MakeAdmission("test-cq").Assignment(cpu, "default", "8").Assignment(memory, "default", "3Gi").Obj()).
					Obj(),
			},
			targetCQ: "test-cq",
			incomingResources: map[corev1.ResourceName]string{
		...
		"preemption scenario: CPU needs borrowing but not preemption, memory needs preemption": {
			clusterQueues: []*kueue.ClusterQueue{
				utiltesting.MakeClusterQueue("queue-no-borrow").
					Cohort("cohort").
					ResourceGroup(*utiltesting.MakeFlavorQuotas("default").
					...
					Obj(),
				utiltesting.MakeClusterQueue("queue-can-borrow").
					Cohort("cohort").
					ResourceGroup(*utiltesting.MakeFlavorQuotas("default").
						Resource(cpu, "10", "10").      // nominal: 10, borrowingLimit: 10
			...
			admitted: []kueue.Workload{
				...
				*utiltesting.MakeWorkload("memory-user", "").
					Queue("q-can-borrow").
					Request(memory, "3Gi").
					ReserveQuota(utiltesting.MakeAdmission("queue-can-borrow").Assignment(memory, "default", "3Gi").Obj()).
					Obj(),
			},
		...
		"preemption scenario: CPU needs borrowing(been borrowed from the cohort) but not preemption, memory needs preemption": {
			clusterQueues: []*kueue.ClusterQueue{
				utiltesting.MakeClusterQueue("queue-cpu-borrowed").
					Cohort("cohort").
					ResourceGroup(*utiltesting.MakeFlavorQuotas("default").
					...
					Obj(),
				utiltesting.MakeClusterQueue("queue-memory-borrowed").
					Cohort("cohort").
					ResourceGroup(*utiltesting.MakeFlavorQuotas("default").
			...
			admitted: []kueue.Workload{
				*utiltesting.MakeWorkload("cpu-borrowed-user", "").
					Queue("queue-cpu-borrowed").
					Request(cpu, "6").
					Request(memory, "3Gi").
					ReserveQuota(utiltesting.MakeAdmission("queue-cpu-borrowed").Assignment(cpu, "default", "6").Assignment(memory, "default", "3Gi").Obj()).
					Obj(),
				*utiltesting.MakeWorkload("memory-borrowed-user", "").
					Queue("queue-memory-borrowed").
					Request(cpu, "3").
					Request(memory, "2Gi").
					ReserveQuota(utiltesting.MakeAdmission("queue-memory-borrowed").Assignment(cpu, "default", "3").Assignment(memory, "default", "2Gi").Obj()).
					Obj(),
			},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			wlBuilder := utiltesting.MakeWorkload("incoming-wl", "")
